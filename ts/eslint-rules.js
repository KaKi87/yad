const mergeConsecutiveExportConst = {
    meta: {
        type: "layout",
        docs: {
...
const getExportedBindingNames = node => {
...
const collectInitReferences = node => {
...
const constReferencesPriorLet = (node, priorLetNames) => {
    if(node.type !== "ExportNamedDeclaration" || node.declaration?.type !== "VariableDeclaration")
        return false;
...
const exportTopAndKindOrder = {
    ...
    create: context => ({
        Program: program => {
            const entries = program.body.filter(node => node.type !== "ImportDeclaration"),
                exportIndices = [];
...
const newlineAfterVarKind = {
    ...
    create: context => {
        ...
        const getLineIndent = lineIndex0 => (sourceCode.lines[lineIndex0] ?? "").match(/^\s*/)?.[0] ?? "";

        const checkVarDecl = variableDeclaration => {
            if(variableDeclaration.declarations.length < 2)
                return;

            const kindToken = sourceCode.getFirstToken(variableDeclaration);
            const firstDeclaratorToken = sourceCode.getFirstToken(variableDeclaration.declarations[0]);
            ...
            const indent = getLineIndent(variableDeclaration.loc.start.line - 1);
            const replacement = `\n${indent}    `;
...
const SPACE_REQUIRED_BEFORE_BRACE = new Set([
    "else",
    "finally",
    "do",
]);

const spaceBeforeElseCatchDoBraces = {
    meta: {
        type: "layout",
        docs: {
    ...
    create: context => {
        ...
        const checkBlock = (block, keyword) => {
            const
        ...
        const checkColonBeforeBlock = block => {
            const
                brace = sourceCode.getFirstToken(block),
...
const getTernaryQuestionToken = (node, sourceCode) =>
...
const getTernaryColonToken = (node, sourceCode) =>
...
const getTernaryChainRoot = node => {
    let current = node;
...
const isPatternMatchingChain = (root, sourceCode) => {
...
const noMultiSpacesExceptPatternTernary = {
    meta: {
        type: "layout",
        docs: {
    ...
    create: context => {
        ...
        const isPatternTernaryPadding = (leftToken, rightToken) =>
            rightToken.value === "?"
...
const ternaryLinebreak = {
    ...
    create: context => {
        ...
        const validatePatternMatching = root => {
            const
...
const isVoidExpression = expression =>
    expression.type === "UnaryExpression" && expression.operator === "void";

const isVoidWrappedWithParens = (expression, sourceCode) => {
...
const formatVoidWrap = (expression, sourceCode) => {
...
const changesVoidSignature = (context, arrowNode, expression) => {
...
const formatConciseExpression = (context, arrowNode, expression, sourceCode) => {
...
const conciseArrowBody = {
    ...
    create: context => {
        ...
        const reportConciseBody = (arrowNode, expression, bodyNode, messageId = "concise") => {
        ...
        const reportVoidWrap = (arrowNode, expression) => {
            context.report({
                node: expression,
                messageId: "voidWrap",
